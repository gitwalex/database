...
open class ObservableTableRow : BaseObservable, CoroutineScope {
    ...
    fun fillContent(c: Cursor) {
        ...
        for (i in 0 until c.columnCount) {
            ...
            when (c.getType(i)) {
                Cursor.FIELD_TYPE_BLOB -> currentContent.put(colName, c.getBlob(i))
    ...
    fun getAsBooleanOrNull(column: String): Boolean? {
        return if (isNull(column)) null else getAsBoolean(column)
    }
    ...
    fun getAsIntOrNull(column: String): Int? {
        return if (isNull(column)) null else getAsInt(column)
    }
